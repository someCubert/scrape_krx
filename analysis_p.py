...
def run_ljung_box(data_series, lags_to_test=10):
    ...
    try:
        lb_results = acorr_ljungbox(data_series, lags=actual_lags, return_df=True)
...
def policy_change_analysis(conn, date_str, event_1, event_2, est_1, est_2, const, name):
    ...
    # prepare data
    query1 = f'SELECT "date", "Issue name", "Close", "Issue code", "No. of listed shares", "No. of shares of foreign ownership", "Foreign ownership ratio", "Foreign ownership limit quantity", "Exhaustion rate" FROM foreign_ownership WHERE "date" <= "{event_after}" AND "date" >= "{event_before}" AND not("Industry" is NULL)'
    ...
    df_calc_t = df_main[['date', 'Issue code', 'CAFO']].copy()
    latest_date = df_calc_t['date'].max()
    df_calc_t = df_calc_t[df_calc_t['date'] == latest_date].copy()
    df_calc_t = df_calc_t.reset_index(drop=True)

    print(f"results for {name}:")
    # autocorrelation test
    num_lags_lb = 10
    lb_pvalue_market, lags_used_market = run_ljung_box(df_calc_t['CAFO'], lags_to_test=num_lags_lb)
    if lb_pvalue_market is not None:
        ...
        print(f"  Ljung-Box test could not be run (N={len(df_calc_t['CAFO'])}).\n")

    # ttest, null hypothesis: mean = 0
    hyp0 = 0
    
    ttest_result_market = stats.ttest_1samp(df_calc_t['CAFO'], hyp0)
    ...
    sample_mean = df_calc_t['CAFO'].mean()
    cohens_d_market = sample_mean / df_calc_t['CAFO'].std()
    
    df_calc_t_KOSPI = df_calc_t[df_calc_t['Issue code'].isin(const)].copy()

    missing_codes = set(const) - set(df_calc_t['Issue code'])
    missing_codes = list(missing_codes)

    print(f"Missing codes in KOSPI200 calculation: {missing_codes}")
    print(f"Number of missing codes in calculation: {len(missing_codes)}")

    ttest_result_KOSPI = stats.ttest_1samp(df_calc_t_KOSPI['CAFO'], hyp0)
    ...
    sample_mean_KOSPI = df_calc_t_KOSPI['CAFO'].mean()
    cohens_d_KOSPI = sample_mean_KOSPI / df_calc_t_KOSPI['CAFO'].std()
    ...
    # print results
    print(f"  [Market] T-Test   t-statistic: {ttest_result_market.statistic}, p-value: {ttest_result_market.pvalue}, df: {ttest_result_market.df}, Mean: {sample_mean:.4f}, Confidence Interval (95%): ({ci_low_market:.4f}, {ci_high_market:.4f}), Cohen's d: {cohens_d_market:.4f}")
    print(f"  [Market] Generalized Sign   p = {p_m:.4f}, q = {q_m}, n = {n_m}, Z = {Z_m:.4f}, p-value = {p_value_m}")
    print(f"  [KOSPI200] T-Test   t-statistic: {ttest_result_KOSPI.statistic}, p-value: {ttest_result_KOSPI.pvalue}, df: {ttest_result_KOSPI.df}, Mean: {sample_mean_KOSPI:.4f}, Confidence Interval (95%): ({ci_low_KOSPI:.4f}, {ci_high_KOSPI:.4f}), Cohen's d: {cohens_d_KOSPI:.4f}")
    print(f"  [KOSPI200] Generalized Sign   p = {p_k:.4f}, q = {q_k}, n = {n_k}, Z = {Z_k:.4f}, p-value = {p_value_k}\n\n")
...
#real
m_pFSCMA,K_pFSCMA = policy_change_analysis(conn, '2009-02-04', 0, 270, -1, -180, KOSPI200_const_FSCMA, 'FSCMA')
m_pShort1,K_pShort1 = policy_change_analysis(conn, '2020-03-13', 0, 180, -1, -180, KOSPI200_const_short1, 'Short1')
m_pShort2,K_pShort2 = policy_change_analysis(conn, '2023-11-06', 0, 180, -1, -180, KOSPI200_const_short2, 'Short2')
m_pLEIs,K_pLEIs = policy_change_analysis(conn, '2023-12-14', 0, 270, -1, -180, KOSPI200_const_LEIs, 'LEIs')
m_pForexW,K_pForexW = policy_change_analysis(conn, '2024-07-01', 0, 270, -181, -360, KOSPI200_const_forex, 'ForexWithPilot')
m_pForex,K_pForex = policy_change_analysis(conn, '2024-07-01', 0, 270, -1, -180, KOSPI200_const_forex, 'Forex')
